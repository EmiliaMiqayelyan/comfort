...
export function DownloadForm({ file }: { file?: DownloadFile }) {
  ...
  const [title, setTitle] = useState<LocalizedString>(file?.title ?? emptyLocalized());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.en.trim() || !filename.trim() || !url.trim()) {
      setError(t("requiredFields"));
      return;
    }
    setSaving(true);
    setError(null);
    const payload = { title, filename, category, url, size, downloadable };
    try {
