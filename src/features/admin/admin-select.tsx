...
export function AdminSelect({
  ...
  return (
    <Select.Root value={value || undefined} onValueChange={onValueChange}>
      <Select.Trigger
        className={cn(
          "flex h-11 w-full items-center justify-between gap-2 rounded-xl border border-white/10",
          "bg-[#0b0f17] px-4 text-sm text-zinc-100 outline-none transition",
      ...
      <Select.Portal>
        <Select.Content
          className={cn(
            "z-[100] overflow-hidden rounded-xl border border-white/10 bg-[#141a24] shadow-2xl",
            ...
            {options.map((option) => (
              <Select.Item
                ...
                className={cn(
                  ...
                  "text-zinc-200 outline-none data-[highlighted]:bg-accent/15 data-[highlighted]:text-accent",
                )}
              >
