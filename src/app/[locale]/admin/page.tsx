          ...
          {cards.map((card) => {
            ...
            return (
              <Link
                key={card.href}
                href={card.href}
                className="group rounded-2xl border border-white/5 bg-[#141a24] p-5 transition hover:border-accent/30 hover:bg-[#171e2a]"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-secondary/15 text-secondary">
                    <Icon className="h-5 w-5" />
                  </div>
                  <ArrowRight className="h-4 w-4 text-zinc-600 transition group-hover:translate-x-0.5 group-hover:text-accent" />
                </div>
                <p className="mt-5 text-sm text-zinc-400">{t(card.labelKey)}</p>
